...
--
-- Host: 127.0.0.1
-- Generation Time: Feb 06, 2019 at 01:55 AM
...
--
-- Stand-in structure for view `viewsalling`
-- (See below for the actual view)
--
CREATE TABLE `viewsalling` (
);
...
--
-- Structure for view `viewsalling`
--
DROP TABLE IF EXISTS `viewsalling`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `viewsalling`  AS  select `pro`.`pname` AS `pname`,`vs`.`total_quantity` AS `total_quantity`,(`vs`.`total_quantity` * `pro`.`unitprice`) AS `total` from (`product` `pro` join `v_salling` `vs` on((`pro`.`pid` = `vs`.`pid`))) ;
...
--
ALTER TABLE `sales`
  MODIFY `salesid` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=1002;
